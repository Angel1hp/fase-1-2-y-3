...
const modalInstalarIos    = document.getElementById('modal-instalar-ios');
const btnCerrarIos        = document.getElementById('btn-cerrar-ios');
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
        if (esMaximizado) {
            ...
            volverALaApp();
            return;
        } else if (modalInstalarIos && !modalInstalarIos.classList.contains('oculto')) {
            modalInstalarIos.classList.add('oculto');
            return;
        }
    }
...
// ── Lógica de Instalación PWA (Móvil / Escritorio) ────────────────────
let deferredPrompt = null;
const esStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;

// Función para detectar si es un dispositivo Apple (iOS / iPadOS)
function esDispositivoIos() {
    return /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
}

// 1. Capturar el prompt nativo para Android / Computadoras
window.addEventListener('beforeinstallprompt', (e) => {
    // Evitar el cartel automático de instalación
    e.preventDefault();
    // Guardar el evento para dispararlo cuando el usuario haga clic
    deferredPrompt = e;
    
    // Si no está corriendo en modo instalado, mostrar el botón
    if (btnInstalar && !esStandalone) {
        btnInstalar.classList.remove('oculto');
    }
});

// 2. Controlar la aparición manual para iOS (Safari no emite beforeinstallprompt) y personalizar texto según SO
window.addEventListener('load', () => {
    const textoInstalar = document.getElementById('texto-instalar');
    if (textoInstalar) {
        if (/Android/.test(navigator.userAgent)) {
            textoInstalar.textContent = 'Instalar APK';
        } else if (esDispositivoIos()) {
            textoInstalar.textContent = 'Instalar en iPhone';
        } else {
            textoInstalar.textContent = 'Instalar App';
        }
    }

    if (esDispositivoIos() && !esStandalone && btnInstalar) {
        btnInstalar.classList.remove('oculto');
    }
});
...
if (btnInstalar) {
    btnInstalar.addEventListener('click', () => {
        // Caso A: Dispositivos con soporte nativo (Android / PC)
        if (deferredPrompt) {
            deferredPrompt.prompt();
            deferredPrompt.userChoice.then((choiceResult) => {
                if (choiceResult.outcome === 'accepted') {
                    console.log('El usuario aceptó la instalación de Mi Lector PDF');
                }
                deferredPrompt = null;
                btnInstalar.classList.add('oculto');
            });
        }
        // Caso B: Dispositivos iOS (iPhone / iPad Safari)
        else if (esDispositivoIos() && !esStandalone) {
            if (modalInstalarIos) {
                modalInstalarIos.classList.remove('oculto');
            }
        }
    });
}

// 4. Cerrar el modal instructivo de iOS
if (btnCerrarIos) {
...
// 5. Ocultar botón si la instalación se concreta desde el navegador
window.addEventListener('appinstalled', (evt) => {
    console.log('Mi Lector PDF ha sido instalado exitosamente!');
    if (btnInstalar) {
        btnInstalar.classList.add('oculto');
    }
});
